"""
@Project   : CubeGirl
@Module    : tree_pos_vecs.py
@Author    : Deco [deco@cubee.com]
@Created   : 12/15/17 5:03 PM
@Desc      : 
"""
...
def dicts2(t, pos):
    return {pos[k]: dicts2(t[k], pos) for k in t}


def tree2syntac(arcs, postags):
    ...
    tree_parse = dicts2(tree_parse, postags)
...
def trees2syns(templates):
    ...
    _, postags_list, arcs_list = \
        Syntactic().parse_many_templates(tpls_clean)
    return [tree2syntac(arcs, postags) for postags, arcs in
            zip(postags_list, arcs_list)]
