"""
@Project   : CubeGirl
@Module    : agent_simple_nodb.py
@Author    : Deco [deco@cubee.com]
@Created   : 1/23/18 3:19 PM
@Desc      : 
"""
import numpy as np
...
class AgentSimpleRLAllActNoDB(RLAgent, SoftDB, BeliefTracker):
    """相当于Context in context.py"""
    # 引出BeliefTracker in belief_tracker.py
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, db=None,
        ...
        if inputtype=='entropy':
            ...
            in_size = sum([len(self.movie_dict.dict[s])+2 for s
                           in dialog_config.inform_slots])
        out_size = len(dialog_config.inform_slots)+1
        self._init_model(in_size, out_size, n_hid=n_hid, learning_rate_sl=lr,
                         batch_size=batch, ment=ment)
        self._name = name
        if _reload:
    ...
    def _print_progress(self,loss):
        ...
        print('Update %d. Avg turns = %.2f . Avg Reward = %.2f . Success Rate = %.2f . Fail Rate = %.2f . Incomplete Rate = %.2f . Loss = %.3f' %
              (self.num_updates, avg_turn, avg_ret, float(n_suc)/tot,
               float(n_fail)/tot, float(n_inc)/tot, avg_loss))
    ...
    def next(self, user_action, verbose=False):
        self._update_state(user_action['nl_sentence'], upd=self.upd,
                           verbose=verbose)
        self.state['turn'] += 1
        ...
        if self.inputtype=='entropy':
            for i,s in enumerate(dialog_config.inform_slots):
                ...
                p_vector[i+len(dialog_config.inform_slots)] = \
        ...
        if self.training and self.num_updates<self.pol_start:
            ...
            if self.training:
                act, action, p_out = self._prob_act(p_vector, db_probs,
                                                    mode='sample')
            else:
                act, action, p_out = self._prob_act(p_vector, db_probs,
                                                    mode='max')
