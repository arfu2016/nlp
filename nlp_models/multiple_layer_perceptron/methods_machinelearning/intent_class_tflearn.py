"""
@Project   : CubeGirl
@Module    : intent_class_tflearn.py
@Author    : Deco [deco@cubee.com]
@Created   : 11/21/17 10:09 PM
@Desc      : 
"""
...
if __name__ == "__main__":
    ...
    layer_0_test = update_input_layer(x_test, vocab_train, vocab_size_train,
                                      train_word2index)
    softreg = LogisticRegression(random_state=0, solver='newton-cg',
                                 max_iter=2000, multi_class='multinomial')
    softreg.fit(layer_0_train, y_train)
    print('Accuracy of the validation data:',
          softreg.score(layer_0_test, y_test), '\n')
    predictions = softreg.predict_proba(layer_0_test).tolist()
